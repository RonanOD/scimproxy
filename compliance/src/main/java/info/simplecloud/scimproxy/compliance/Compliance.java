...
/*		// if oauth2, grant_type client_credentials
		csp.setAuthentication("oauth2");
		csp.setUsername("usr");
		csp.setPassword("pw");
		csp.setOAuth2AuthorizationServer("oAuthAuthorizationServer");

		// if oauth2, other grant_type
		csp.setAuthentication("oauth2-v10");
		csp.setUsername("usr");
		csp.setPassword("pw");
		csp.setOAuth2AuthorizationServer("oAuthAuthorizationServer");
...
*/
		...
		try {
			...
			// TODO: add the required attributes in userSchema and groupSchema that server wanted
			...
			if(e instanceof CritialComplienceException) {
				...
				// TODO: if config succeeds and schema fails we will have more then 1 test!
