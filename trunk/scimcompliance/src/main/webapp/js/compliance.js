    ...
    var handleError = function(event) {
      ...
      $("#compliance-error-container").show();
		...
		var handleResponse = function(data) {
	      ...
	      $('#authenticationDialog').modal("show");
		    ...
		    html = Mustache.to_html(template, {total: failed + success, success: success, failed: failed});
		    $("#compliance-statistics-text").html(html);
		    
		    var options = { legend: 'none', chartArea:{width:"100%",height:"100%"}, colors:['darkgrey','lightgrey'], backgroundColor: "whiteSmoke" };
		    var chartData = google.visualization.arrayToDataTable([ ['Result', 'Number'], ['Failed', failed], ['Success', success] ]);
      ...
      $("#authMethod option:selected").each(function () {
		...
		var sendRequest = function() {
		  var data = { url: $("#complianceUrl").val(),
      ...
      $("#result-container").hide();
		  ...
		  $("#spinner-container").spin({ lines: 13, length: 30, width: 10, radius: 40, rotate: 0, color: '#000', speed: 1, trail: 60, shadow: false, hwaccel: false, className: 'spinner', zIndex: 2e9, top: 'auto', left: 'auto' });
